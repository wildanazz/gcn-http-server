...
def main():
    ...
    a_tilde = adjacency_matrix + eye(22470, dtype='float32')
    ...
    d = diags(np.array(facebook_features.sum(
        axis=1)).flatten())  # degree matrix
    degrees_inverse = np.power(d.diagonal(), -1)
    d_inverse = diags(degrees_inverse)  # degree matrix inverse
    facebook_features = d_inverse.dot(facebook_features)

    # Convert to tensor
    facebook_features = torch.FloatTensor(
        np.array(facebook_features.todense()))
    facebook_train_target = torch.LongTensor(
        np.where(facebook_train_target)[1])
    facebook_validation_target = torch.LongTensor(
        np.where(facebook_validation_target)[1])
    ...
    # Print output
    # print(f'facebook_features: {facebook_features}')
    # print(f'facebook_train_target: {facebook_train_target.size()}')
    # print(f'facebook_validation_target: {facebook_validation_target.size()}')
    # print(f'facebook_test_target: {facebook_test_target.size()}')
    ...
    logging.info(
        f'Test loss: {test_loss.item():.4f} || Test accuracy: {test_accuracy.item():.4f}')

    np.save('./temp/train_losses', train_losses)
    np.save('./temp/train_accuracies', train_accuracies)
    np.save('./temp/validation_losses', validation_losses)
    np.save('./temp/validation_accuracies', validation_accuracies)
    ...
    np.save('./temp/x_embedded', x_embedded)
